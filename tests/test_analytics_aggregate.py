...
def _gs(poss_secs, pu, terr, match_secs):
    return GameStats(game_id=0, date="", opponent="", poss_secs_us=poss_secs,
                     ...
                     match_secs_us=match_secs)

def test_aggregate_two_games():
    a = _gs(600.0, 10, Territory(20.0, 30.0, 50.0), 3000)
    b = _gs(300.0, 6, Territory(40.0, 30.0, 30.0), 3000)
    s = aggregate([a, b])
    assert s.games == 2
...
def test_aggregate_pct_none_without_match_length():
    a = _gs(600.0, 10, Territory(20.0, 30.0, 50.0), 0)
    s = aggregate([a])
    assert s.poss_pct_us is None

def test_aggregate_empty():
    s = aggregate([])
    assert s == SeasonStats(0, 0.0, None, 0, 0, 0, 0, 0, 0, Territory(0.0, 0.0, 0.0))
