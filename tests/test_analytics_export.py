...
def _sample():
    g = GameStats(game_id=1, date="2026-09-12", opponent="Latterman",
                  poss_secs_us=600.0, poss_pct_us=20.0, passes_us=7, shots_us=2,
                  shots_them=1, box_us=1, att_third_us=1, packing_us=1,
                  territory_us=Territory(30.0, 30.0, 40.0), sequences_us=9,
                  match_secs_us=3000)
    season = SeasonStats(1, 600.0, 20.0, 7, 2, 1, 1, 1, 1, Territory(30.0, 30.0, 40.0), 9)
    return [g], season
...
def test_csv_has_header_row_and_game(tmp_path):
    ...
    assert lines[0].startswith("date,opponent,sequences,ball_control_min,possession_pct,passes")
    assert "Latterman" in lines[1]
    assert "10.0" in lines[1]             # 600s -> 10.0 min
    assert "20.0" in lines[1]             # possession % of match
